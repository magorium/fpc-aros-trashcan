...
uses
  ctypes, chelpers, Classes, SDL, collision;
...
//  memset(img, 0, sizeof(img));
  ...
  begin
    ...
    ra[i].pm := self; // this;
    busy[i] := true;
    kids[i] := SDL_CreateThread(PInt(@ext_render_thread),ra+i);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      pa[tn].threadid := tn;
      pa[tn].pm := self; // this;
      kids[tn] := SDL_CreateThread(PInt(@ext_photon_thread),pa+tn);
    end;

    for tn :=0 to Pred(threads) do // ; tn++) // wait for all threads
      if assigned( kids[tn] )
        then SDL_WaitThread(kids[tn], ret);
