...
uses
  {$IFDEF TRACE}heaptrc,{$ENDIF}
  {$IFDEF AROS}arosc_static_link,{$ENDIF}  
  sysutils, math,
  simpledebug,
  ctypes, chelpers,
  pixelina, sjui, font,
...
//  NICEME;
  ...
  thread := SDL_CreateThread(PInt(@run_pixel_machine), nil);  // another stupidity declaration... a PInt ... really ????
...
begin
  {$IFDEF AROS}
  arosc_static_link.AROSC_Init(@PixelMachine_Startup);
  {$ELSE}
